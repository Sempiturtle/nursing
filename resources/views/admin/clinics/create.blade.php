    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-12 animate-slide-up-fade stagger-1">
            <h1 class="text-4xl lg:text-5xl font-black text-white tracking-tighter font-outfit uppercase italic mb-3">
                Node <span class="gradient-text tracking-normal not-italic lowercase font-medium opacity-80">(initialization)</span>
            </h1>
            <p class="text-slate-500 font-medium flex items-center gap-2 text-sm">
                ...
                Register a new care facility node in the localized community network.
            </p>
        </div>

        <form action="{{ route('admin.clinics.store') }}" method="POST" class="animate-slide-up-fade stagger-2">
            @csrf
            <div class="glass rounded-[2.5rem] border-white/5 p-10 lg:p-12 space-y-12">
                ...
                <div class="space-y-8">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="text" name="address" required placeholder="Physical location sector..." 
                                   class="w-full bg-slate-900/50 border border-white/5 rounded-2xl px-6 py-4 text-white font-outfit text-sm focus:ring-2 focus:ring-accent-emerald/20 focus:border-accent-emerald/40 transition-all placeholder:text-slate-700">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div class="space-y-3">
                            ...
                            <input type="text" name="contact_number" required placeholder="Primary uplink number..." 
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="text" name="operational_hours" required placeholder="0800 - 1700 SMT..." 
                                   class="w-full bg-slate-900/50 border border-white/5 rounded-2xl px-6 py-4 text-white font-outfit text-sm focus:ring-2 focus:ring-accent-emerald/20 focus:border-accent-emerald/40 transition-all placeholder:text-slate-700">
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <textarea name="services" required rows="3" placeholder="Define medical services provided at this node..."
                                  class="w-full bg-slate-900/50 border border-white/5 rounded-2xl px-6 py-4 text-white font-outfit text-sm focus:ring-2 focus:ring-accent-emerald/20 focus:border-accent-emerald/40 transition-all placeholder:text-slate-700 resize-none"></textarea>
                    </div>
                </div>
                ...
                <div class="p-8 bg-slate-900/50 rounded-3xl border border-white/5 space-y-4">
                    <div class="flex gap-4">
                        ...
                        <div class="space-y-1">
                            <h4 class="text-xs font-black text-white font-outfit uppercase tracking-widest">Medical Grid Synchronization</h4>
                            <p class="text-[11px] text-slate-500 font-medium italic">Node will be mapped across the care network and visible to all authorized community nodes immediately upon activation.</p>
                        </div>
                    </div>
                </div>
