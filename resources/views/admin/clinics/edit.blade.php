<x-admin-layout>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h1 class="text-2xl font-bold text-zinc-900 tracking-tight font-outfit">Edit Clinic</h1>
                <p class="text-sm text-zinc-500">Update clinic details and info</p>
            </div>
        </div>

        <div class="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-hidden animate-fade-up">
            <form action="{{ route('admin.clinics.update', $clinic) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-2">
                        ...
                        <input type="text" name="address" value="{{ old('address', $clinic->address) }}" required
                               class="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 text-zinc-900 focus:ring-2 focus:ring-[var(--maternal-rose)] focus:border-transparent transition"
                    ...
                    <div>
                        ...
                        <input type="text" name="operating_hours" value="{{ old('operating_hours', $clinic->operating_hours) }}"
                               class="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 text-zinc-900 focus:ring-2 focus:ring-[var(--maternal-rose)] focus:border-transparent transition"
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="services" rows="3"
                                  class="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 text-zinc-900 focus:ring-2 focus:ring-[var(--maternal-rose)] focus:border-transparent transition"
                                  placeholder="e.g., Breastfeeding counseling, Newborn checkups...">{{ old('services', $clinic->services) }}</textarea>
                    </div>
                </div>
