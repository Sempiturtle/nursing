<x-admin-layout>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h1 class="text-2xl font-bold text-zinc-900 tracking-tight font-outfit">Add New Video</h1>
                <p class="text-sm text-zinc-500">Upload a resource to the educational library</p>
            </div>
        </div>

        <div class="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-hidden animate-fade-up">
            <form action="{{ route('admin.videos.store') }}" method="POST" class="p-8">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="category" required
                            ...
                            <option value="Infant Care" {{ old('category') == 'Infant Care' ? 'selected' : '' }}>Infant Care</option>
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="description" rows="4" required
                                  class="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 text-zinc-900 focus:ring-2 focus:ring-[var(--maternal-rose)] focus:border-transparent transition"
                                  placeholder="Provide a brief overview of the video content...">{{ old('description') }}</textarea>
