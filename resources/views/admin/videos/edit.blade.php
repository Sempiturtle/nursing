    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-12 animate-slide-up-fade stagger-1">
            <h1 class="text-4xl lg:text-5xl font-black text-white tracking-tighter font-outfit uppercase italic mb-3">
                Asset <span class="gradient-text tracking-normal not-italic lowercase font-medium opacity-80">(recalibration)</span>
            </h1>
            <p class="text-slate-500 font-medium flex items-center gap-2 text-sm">
                ...
                Refining clinical walkthrough parameters within the media intelligence grid.
            </p>
        </div>

        <form action="{{ route('admin.videos.update', $video) }}" method="POST" enctype="multipart/form-data" class="animate-slide-up-fade stagger-2">
            @csrf @method('PUT')
            <div class="glass rounded-[2.5rem] border-white/5 p-10 lg:p-12 space-y-12">
                ...
                <div class="space-y-8">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div class="space-y-3">
                            ...
                            <input type="text" name="title" value="{{ old('title', $video->title) }}" required placeholder="Asset nomenclature..." 
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="relative group">
                                <select name="category" required
                                    ...
                                    <option value="Interview" {{ old('category', $video->category) == 'Interview' ? 'selected' : '' }}>Interview</option>
                                    <option value="Workshop" {{ old('category', $video->category) == 'Workshop' ? 'selected' : '' }}>Workshop</option>
                                    <option value="Quick Tip" {{ old('category', $video->category) == 'Quick Tip' ? 'selected' : '' }}>Quick Tip</option>
                                </select>
                    ...
                    <div class="space-y-3">
                        ...
                        <textarea name="description" required rows="4" placeholder="Provide technical overview of this procedure..."
                                  class="w-full bg-slate-900/50 border border-white/5 rounded-2xl px-6 py-4 text-white font-outfit text-sm focus:ring-2 focus:ring-accent-rose/20 focus:border-accent-rose/40 transition-all placeholder:text-slate-700 resize-none">{{ old('description', $video->description) }}</textarea>
                    </div>

                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div class="space-y-3">
                            <label class="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Transmission Path (URL)</label>
                            <input type="url" name="url" value="{{ old('url', $video->url) }}" required placeholder="https://youtube.com/watch?v=..." 
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="url" name="thumbnail" value="{{ old('thumbnail', $video->thumbnail) }}" placeholder="https://cdn.maternal.io/thumbnails/..." 
                                   class="w-full bg-slate-900/50 border border-white/5 rounded-2xl px-6 py-4 text-white font-mono text-[10px] focus:ring-2 focus:ring-accent-rose/20 focus:border-accent-rose/40 transition-all placeholder:text-slate-700">
                        </div>
                    </div>
                </div>
                ...
                <div class="p-8 bg-slate-900/50 rounded-3xl border border-accent-amber/20 space-y-4">
                    <div class="flex gap-4">
                        ...
                        <div class="space-y-1">
                            <h4 class="text-xs font-black text-white font-outfit uppercase tracking-widest">Awaiting Re-Authorization</h4>
                            <p class="text-[11px] text-slate-500 font-medium italic">Modifying this asset will update the broadcast protocol across all maternal nodes. Verify asset path and cluster alignment before committing.</p>
                        </div>
                    </div>
                </div>
